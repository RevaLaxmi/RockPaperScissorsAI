...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            if player_move != "Unknown" and player_move != last_player_move:
                # AI only picks a move when player changes gesture
                ai_move = random.choice(MOVES)
                last_player_move = player_move  # Lock player move

            if player_move != "Unknown":
                winner_text = get_winner(player_move, ai_move)
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            if recognized_move != "Unknown" and recognized_move != last_player_move:
                # AI only picks a move when player finalizes theirs
                ai_move = random.choice(MOVES)
    ...
    # AI Move only appears after player finalizes move
    if show_ai_move:
        cv2.putText(frame, f"AI Move: {ai_move}", (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        cv2.putText(frame, winner_text, (50, 200), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), 3)
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            # If the player starts forming a new move, reset AI move visibility
            if recognized_move == "Unknown":
                show_ai_move = False
                ai_move = "🤔"
                winner_text = ""
